...
- 状态：已完成
- 当前代码状态：FastAPI SSE 流式消息接口、前端 SSE 消费、token 输出和压缩提醒事件已成为当前对话主链路
- 本文档作用：说明 `/messages/stream` 的流式体验、事件语义和错误策略
- 仍有效内容：前端消费项目侧稳定 SSE 协议，普通完成后 assistant 消息落盘，压缩提醒不暴露摘要正文
- 后续关注：独立 run event stream、断线重连等能力如有需要应另行设计，不复用旧占位接口
