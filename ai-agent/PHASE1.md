...
- 状态：已完成
- 当前代码状态：本地登录、多用户隔离、多 Session、Session 文件投影和基础聊天前端已经形成稳定基座
- 本文档作用：说明当前仍有效的产品层 Session 基座和文件投影规则
- 仍有效内容：`session_id -> thread_id` 绑定、多用户目录隔离、Session 生命周期、产品层原始消息保存
- 后续关注：后续阶段继续复用这套产品层基座，不在本文档扩展新的 runtime 或 DBAAS tool 细节
