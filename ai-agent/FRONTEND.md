...
- 当前用户登录后，主会话列表只展示 `user_id` 和 `role` 均匹配当前身份的 Session
...
如果 `GET /api/v1/sessions` 返回 `stale_identity_items`，
前端可以不展示；如需支持清理，可单独展示为旧身份会话分组。
旧身份会话只展示标题、预览、旧 `role/user` 和删除按钮；
不展示打开入口，也不允许继续对话、审批或任务订阅。
