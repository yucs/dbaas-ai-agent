...
def get_service(name: str, request: Request) -> ServiceDetailResponse:
    """按服务组名称查询完整服务详情。"""

    store = get_store(request)
    return ServiceDetailResponse.model_validate(_get_service_or_404(store, name))
...
def list_services(
    ...
    return [
        ...
        for service_detail in store.list_service_details(owner=owner)
    ]
...
def _get_service_or_404(store: JsonDataStore, name: str) -> dict:
    """读取服务详情，不存在时抛出 404。"""

    service_detail = store.get_service_detail(name)
